...
impl MCPClient {
    ...
    async fn send_request(&self, method: &str, params: Option<JsonValue>) -> Result<JsonValue> {
        ...
        tracing::debug!("MCP request: {} (id={})", method, id);
    ...
    pub async fn connect(&self) -> Result<()> {
        if !self.is_connected().await {
            self.client
                ...
                .send()
                .await
                .context("MCP server health check failed")?;
        }

        *self.connected.write().await = true;
        tracing::info!("Connected to MCP server at {}", self.server_url);
        Ok(())
    }
    ...
    pub async fn initialize(&self, client_name: &str, client_version: &str) -> Result<ServerCapabilities> {
        ...
        tracing::debug!("MCP server info: {} v{}", init_result.server_info.name, init_result.server_info.version);
        tracing::debug!("MCP server capabilities: {:?}", init_result.capabilities);
    ...
    pub async fn list_tools(&self) -> Result<Vec<Tool>> {
        ...
        tracing::debug!("Listed {} tools from MCP server", tools_result.tools.len());
    ...
    pub async fn call_tool(&self, name: &str, arguments: JsonValue) -> Result<Vec<ContentBlock>> {
        ...
        tracing::debug!("Tool {} returned {} content blocks", name, call_result.content.len());
